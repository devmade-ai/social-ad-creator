...
// Bridge fontEmbed's per-weight / per-font warning callback to the structured
// debug log. Silent degradation in the field is what produced the original
// CORS noise — surfacing partial failures preserves observability.
const logFontWarning = (msg) =>
  debugLog('export', 'font-embed-warning', { msg }, 'warn')
...
export async function captureAsBlob(element, width, height, format, fontIds = []) {
  const fontEmbedCSS = await getEmbeddedFontCSS(fontIds, { onWarning: logFontWarning })
