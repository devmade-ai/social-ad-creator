  ...
  if (confirming) {
    return (
      <div className="flex items-center gap-1">
        <button
          onClick={handleClick}
          className={confirmClassName || 'px-2 py-1 text-xs font-medium rounded-md bg-red-500 text-white hover:bg-red-600 active:scale-95 transition-all'}
        >
          {confirmLabel}
        </button>
        <button
          onClick={(e) => { e.stopPropagation(); setConfirming(false) }}
          className="px-2 py-1 text-xs font-medium rounded-md bg-ui-surface-hover text-ui-text-muted hover:bg-ui-surface-inset transition-all"
        >
          Cancel
        </button>
      </div>
