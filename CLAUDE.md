...
- **Z-index scale:** Canvas internals 0-10, sticky headers 20, sheets/drawers 30, mobile nav 40, menu backdrop 40, menu dropdown 50, modals use native `<dialog>` top layer (no z-index needed), toasts `z-[70]`, debug 80. DebugPill uses inline `zIndex: 80`. **Two intentional deviations from glow-props Z_INDEX_SCALE.md:** (1) Modals use native `<dialog>` which renders in the browser top layer, above all z-indexes — this inverts the scale's "debug pill is always topmost" rule when a modal is open. Accepted because native `<dialog>` provides built-in focus trap, Escape handling, and inert background that the pattern's div-based z-60 modal cannot match; the debug pill's crash-survival purpose still holds when no modal is open. (2) MobileNav sits at z-40 (same layer as menu backdrop) rather than z-20 (sticky header per scale) — this works because the menu backdrop covers MobileNav when the menu is open (desired behavior: nav non-interactive while menu open). A stricter alignment would move MobileNav to z-30 (sheet/drawer class), but current z-40 is stable and tested.
