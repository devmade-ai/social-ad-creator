...
- **pdf-lib image handling:** pdf-lib embeds PNG directly (FlateDecode — no re-encoding). PDF export uses pixelRatio:1 with 1:1 pixel-to-point page dimensions so PDF viewers don't resample the image. Previous approach (pixelRatio:2 with 72/96 pt conversion) forced viewer downsampling which destroyed smooth gradients (vignettes looked ~30% quality). Diagnostic image download enabled in dev mode.
